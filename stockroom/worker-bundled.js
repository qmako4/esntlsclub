// ESNTLS site edge worker + STOCKROOM api — SINGLE FILE BUILD.
//
...
function netFor(platform, costPence, resalePence, postagePence) {
...
function rankPlatforms(platforms, categories, costPence, resalePence, postagePence) {
  return platforms
    .map((platform) => {
...
function isStockroomPath(pathname) {
  return pathname === '/api/stockroom' || pathname.startsWith('/api/stockroom/');
}

async function handleStockroom(request, env) {
