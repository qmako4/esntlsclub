...
5. OpenAI performs a simple grass background swap while preserving the original product, hand, angle, scale, and foreground details.
...
The prompt tells OpenAI to replace only the background with this grass photo, keep the product and any hand unchanged, avoid zooming or restyling, and use only a subtle natural grounding shadow when needed.
